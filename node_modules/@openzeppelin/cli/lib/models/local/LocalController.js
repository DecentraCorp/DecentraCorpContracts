...
class LocalController {
    ...
    initProjectFile(name, version, force = false, publish) {
        ...
        upgrades_1.Loggy.noSpin(__filename, 'initProjectFile', 'init-project-file', `Project initialized. Write a new contract in the contracts folder and run 'openzeppelin create' to deploy it.`);
    }
    bumpVersion(version) {
    ...
    checkCanAdd(contractName) {
        ...
        if (!upgrades_1.FileSystem.exists(path)) {
    ...
    hasBytecode(contractDataPath) {
        if (!upgrades_1.FileSystem.exists(contractDataPath))
            return false;
        const bytecode = upgrades_1.FileSystem.parseJson(contractDataPath).bytecode;
    ...
    getContractSourcePath(contractAlias) {
        ...
        if (contractName) {
            ...
            const { compiler, sourcePath } = upgrades_1.FileSystem.parseJson(contractDataPath);
    ...
    linkDependencies(dependencies, installDependencies = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (linkedDependencies.length > 0) {
                ...
                upgrades_1.Loggy.noSpin(__filename, 'linkDependencies', 'link-dependencies', `${label} linked to the project. Run 'openzeppelin create' to deploy one of its contracts.`);
            }
        });
    }
    // DependencyController
