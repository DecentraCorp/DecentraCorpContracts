...
exports.DISABLE_INTERACTIVITY = !!process.env.OPENZEPPELIN_NON_INTERACTIVE
...
function promptIfNeeded({ args = {}, opts = {}, defaults, props }, interactive) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const argsAndOptsQuestions = Object.keys(argsAndOpts)
            .filter(name => argsAndOpts[name] === undefined || (typeof argsAndOpts[name] !== 'boolean' && lodash_isempty_1.default(argsAndOpts[name])))
...
function proxiesList(pickProxyBy, network, filter, projectFile) {
    ...
    const list = Object.keys(groupedByPackage).map(packageName => {
        ...
        return [new inquirer_1.default.Separator(` = ${separator} =`), ...lodash_uniqby_1.default(packageList, 'name')];
    });
...
function contractsList(name, message, type, source) {
    ...
    if (!source || source === 'built') {
        ...
        return inquirerQuestion(name, message, type, [...contractsFromBuild, ...lodash_flatten_1.default(packageContracts)]);
    }
    else
        return [];
}
...
function answersFor(inputs, questions, props, interactive) {
    return __awaiter(this, void 0, void 0, function* () {
        const merged = interactive ? Object.assign({}, inputs, (yield inquirer_1.default.prompt(questions))) : inputs;
